...
import torch
import torch as th
from litsr.data.srmd_degrade import SRMDPreprocessing, BicubicPreprocessing
...
from archs.CNN-DiffSR_unet_arch import *
...
class GaussianDiffusion(nn.Module):
    ...
    def super_resolution(self, x_in, continous=False):
        return self.p_sample_loop(x_in, continous=continous)
...
class DiffSRModel(pl.LightningModule):
    def __init__(self, opt):
        ...
        self.degrade = BicubicPreprocessing(
            self.opt.scale
        )
        self.valid_degrade = BicubicPreprocessing(
            self.opt.scale
        )
    ...
    def training_step(self, batch):
        ...
        # Bicubic_interpolation = nn.Upsample(scale_factor=self.opt.scale, mode='bicubic', align_corners=True)
        # bic = Bicubic_interpolation(im_q)
        ...
        if self.current_epoch < self.pretrain_epochs:
            ...
            sr = self.diffusion.super_resolution(hr_)
            loss_sr_pixel = F.l1_loss(sr, hr)

            loss = loss_consis + loss_sr_pixel
            loss_elbo = 0
        else:
            ...
            l_pix = self.diffusion({"HR": hr, "SR": hr_})
            loss_elbo = l_pix.sum() / int(b * c * h * w)  # noise loss

            #sr = self.diffusion.super_resolution(hr_)  # sr loss
            #loss_sr_pixel = F.l1_loss(sr, hr)
        ...
        #self.log("train/loss_sr_pixel", loss_sr_pixel)
    ...
    def validation_epoch_end(self, outputs):
        ...
        f = open('./logs/cnn-diffsr_x4/validation_epoch_end.txt', 'a')
