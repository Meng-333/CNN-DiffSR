...
class bicubic(nn.Module):
    ...
    def contribute(self, in_size, out_size, scale):
        ...
        indice0 = left0.unsqueeze(1) + torch.arange(start=0, end=P).to(
            torch.float32
        ).unsqueeze(0)
        indice1 = left1.unsqueeze(1) + torch.arange(start=0, end=P).to(
            torch.float32
        ).unsqueeze(0)
        ...
        indice0 = torch.min(
            torch.max(torch.FloatTensor([1]), indice0),
            torch.FloatTensor([in_size[0]]),
        ).unsqueeze(0)
        indice1 = torch.min(
            torch.max(torch.FloatTensor([1]), indice1),
            torch.FloatTensor([in_size[1]]),
        ).unsqueeze(0)
    ...
    def forward(self, input, scale=1 / 4):
        ...
        weight0 = weight0.to(input.device)
        weight1 = weight1.to(input.device)
        indice0 = indice0.to(input.device)
        indice1 = indice1.to(input.device)
...
class SRMDPreprocessing(nn.Module):
    def __init__(
        self,
        scale,
        mode="bicubic",
    ...
    @torch.no_grad()
    def forward(self, hr_tensor, random=True):
        flag = False
        if len(hr_tensor.shape) == 4:
            hr_tensor = hr_tensor.unsqueeze(1)
            flag = True
        ...
        if flag:
            return (
                lr_blured.view(B, C, H // int(self.scale), W // int(self.scale)),
                b_kernels,
            )
        else:
            return (
...
class BicubicPreprocessing(nn.Module):
    def __init__(
        self,
        scale,
        mode="bicubic",
